...
class Player extends Human implements CommandSender, InventoryHolder, ChunkLoader, IPlayer, NetworkSession{
	...
	protected function processMovement($tickDiff){
		...
		if(($distanceSquared / ($tickDiff ** 2)) > 120 and !$this->allowMovementCheats){
				...
				if($snowball instanceof Projectile){
					...
					$snowball->spawnToAll();
                            ...
                            if(floor(($time = microtime(true)) - $this->lastEnderPearlUse) >= 1){
                                ...
                                $splashpotion = new ThrownPotion($this->getLevel(), $nbt, $this);
                                $splashpotion->setMotion($splashpotion->getMotion()->multiply($f));
                                if($this->isSurvival()){
                                    $item->setCount($item->getCount() - 1);
                                    $this->inventory->setItemInHand($item->getCount() > 0 ? $item : Item::get(Item::AIR));
                                }
                                $this->server->getPluginManager()->callEvent($ev = new ProjectileLaunchEvent($splashpotion));
                                if($ev->isCancelled()){
                                        $splashpotion->kill();
                                }else{
	...
	public function sendTip($message){
        ...
        public function sendSubTitle($message){
		$pk = new SetTitlePacket();
                $pk->fadeInTime = 1;
                $pk->fadeOutTime = 1;
                $pk->stayTime = 1;
